...
The SurveyName variable is used only in some titles on alumni survey outputs, it is not vital for data access: put whatever you to appear as the description for the survey. There is not an equivalent for the baseline survey at the moment because there are no key variable analysis documents, but you could easily add 'SurveyName_base' and declare a variable for it (in the styke above).

# [2] Library calls

The next step is to call the various packages that will help us access and analyse the data. The easiest way to do this is through pacman, a useful package-managing package (!). The first line of code checks if you have pacman installed and, if not, installs it from [CRAN](https://cran.r-project.org/). The second line calls the package itself and loads it. The third line uses pacman's p_lod function to check and load the other packages used in this script.   
