...
overview_score <- alumni.data %>% group_by(Scheme,SchemeType) %>% filter(!Scheme=="SS", !Scheme=="CD") %>% score_summary
...
empchange_sch <- alumni.data %>% filter(!CurrentJobChange=="NA") %>% group_by(Scheme) %>% count(CurrentJobChange) %>% mutate(prop = round((n / sum(n))*100,1)) %>% arrange(Scheme,CurrentJobChange)
...
empreturnorg_sch <- alumni.data %>% filter(grepl("_Two$", SurveyID)) %>% group_by(Scheme) %>% count(ReturnOrganisation) %>% mutate(prop = round((n / sum(n))*100,1)) %>% arrange(Scheme, desc(ReturnOrganisation))
...
empreturntime_sch <- alumni.data %>% filter(grepl("_Two$", SurveyID), !ReturnOrganisation=="Yes", CurrentEmploy=="Employed") %>% group_by(Scheme) %>%  count(ReturnEmploy) %>% mutate(prop = round((n / sum(n))*100,1))
