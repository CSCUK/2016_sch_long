---
title: '2016 Alumni surveys: Gender Breakdown'
author: "CSC E&M Team"
...
```{r, echo=FALSE}
overview_gender %>% 
  select(-Variable,Gender=Response,Cases=freq, 'Proportion (%)'=prop) %>% 
  pander
```
...
```{r, echo=FALSE}
bind_rows(rescountry_gender,resregion_gender) %>% 
...
empcurrent_gender %>% 
  ...
  select(Gender, Employed, contains("Employed and studying"),Studying, Other) %>% 
  pander
```
...
acapostaward_gender %>% 
  filter(Response=="Yes") %>%
  arrange(desc(prop)) %>% 
  select( -Response,-Variable, "Proportion (%)"=prop,"Cases"=freq) %>% 
  pander
```
...
acaquallevel_gender %>% 
  select(-freq, -Variable) %>% 
  spread(Response,prop) %>%
  select(Gender, Doctorate,contains("Master's degree"),contains("Professional accreditation"),contains("Technical training"),
...
```{r, echo=FALSE}
bind_rows(ldrbudget_gender, ldrmanaging_gender, ldrsupervise_gender,ldrstrategy_gender) %>% 
  filter(Response=="Yes") %>% 
  select(-freq, -Response) %>% 
...
resmain_gender %>% 
  filter(Response=="Yes") %>%
  arrange(desc(prop)) %>% 
  select(-Response,-Variable,Cases=freq, "Proportion (%)"=prop) %>% 
  pander
```
...
```{r, echo=FALSE}
bind_rows(reseditor_gender,resreviewer_gender,resorganiser_gender) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>%
  spread(Variable, prop) %>% 
  select(Gender,"Journal Editor"=ResEditor,"Peer reviewer"=ResReviewer,"Conference organiser" = ResOrganiser) %>%
  pander
...
```{r, echo=FALSE}
rbind(teachschool_gender,teachundergrad_gender,teachpostgrad_gender,teachdoctorate_gender,teachtvet_gender) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>% 
  spread(Variable,prop) %>% 
  select(Gender,Doctoral=TeachDoctorate,Postgraduate=TeachPostgrad,Undergraduate=TeachUndergrad,TVET=TeachTVET,School=TeachSchool) %>% 
  pander
...
```{r, echo=FALSE}
bind_rows(impinstitutional_gender,implocal_gender,impnational_gender,impinternational_gender) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>% 
  spread(Variable, prop) %>% 
  select(Gender, Institutional=ImpInstitutional,Local=ImpLocal,National=ImpNational,International=ImpInternational) %>%
  pander
...
```{r, echo=FALSE}
bind_rows(impsocial_gender,impcivic_gender,impecon_gender,imppolicy_gender) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>% 
  spread(Variable, prop) %>%
  rename("Civic Engagement"=ImpCivic, "Economic Development" = ImpEcon,
...
```{r, echo=FALSE}
bind_cols(select(indexskills_gender,Gender, Skills=Mean),
          select(indexldr_gender, Leadership=Mean),
          select(indexresearch_gender, "Research productivity"=Mean),
          select(indexcollab_gender, "Research collaboration"=Mean)) %>% 
  pander
