...
ldrbudget_score %>% 
  filter(!is.na(Response)) %>% 
  arrange(desc(Response)) %>% 
  select(-Variable, "Overseeing Budgets"=Response, Cases=Freq) %>%
  pander
...
ldrmanaging_score %>% 
  filter(!is.na(Response)) %>% 
  arrange(desc(Response)) %>% 
  select(-Variable, "Managing"=Response, Cases=Freq) %>%
  pander
...
ldrstrategy_score %>% 
  filter(!is.na(Response)) %>% 
  arrange(desc(Response)) %>% 
  select(-Variable, "Setting strategy"=Response, Cases=Freq) %>%
  pander
...
ldrsupervise_score %>% 
  filter(!is.na(Response)) %>% 
  arrange(desc(Response)) %>% 
  select(-Variable, "Supervising"=Response, Cases=Freq) %>%
  pander
...
```{r, echo=FALSE}
bind_rows(reseditor_score,resreviewer_score,resorganiser_score) %>% 
  filter(Response=="Yes") %>% 
...
```{r, echo=FALSE}
rbind(teachschool_score,teachundergrad_score,teachpostgrad_score,teachdoctorate_score,teachtvet_score) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>% 
  spread(Variable,prop) %>% 
...
```{r, echo=FALSE}
bind_rows(impinstitutional_score,implocal_score,impnational_score,impinternational_score) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>% 
  spread(Variable, prop) %>% 
...
```{r, echo=FALSE}
bind_rows(impsocial_score,impcivic_score,impecon_score,imppolicy_score) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>% 
  spread(Variable, prop) %>%
...
```{r, echo=FALSE}
bind_cols(select(indexskills_score, Subject=JacsCat, Skills=Mean),
          select(indexldr_score, Leadership=Mean),
          select(indexresearch_score, "Research productivity"=Mean),
          select(indexcollab_score, "Research collaboration"=Mean)) %>% 
  pander
...
```{r, echo=FALSE}
rmarkdown::render("2016_sch_long_subject.Rmd",output_file="dave",output_format='pdf_document')
```
