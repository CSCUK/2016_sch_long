---
title: "2016 Scholar Alumni surveys response rates Analysis"
author: "Matt Mawer"
output:
...
This paper relates to the `r SurveyName`. Basic data on response rates is presented in the sections below, including comparisons within gender, scholar origin, PhD / non-PhD scholars, and year groups within the evaluation framework. Incidence of failed email contacts are highlighted separately in Section 2.
...
As noted above, there were `r resp.overall %>% filter(Response=="Failed") %>% select(n)` failed email addresses (`r resp.overall %>% filter(Response=="Failed") %>% select(Rate)`%). The sections below give a breakdown of failed emails by various categories.
