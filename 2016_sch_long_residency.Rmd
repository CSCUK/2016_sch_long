---
title: '2016 Alumni surveys: Residency Region Breakdown'
author: "CSC E&M Team"
...
```{r, echo=FALSE}
overview_resreg %>% 
  select(-Variable,"Residency Region"=Response,Cases=freq, 'Proportion (%)'=prop) %>% 
  pander
```

# 2. Current residency

This interpretation of this table is more complex for this variable. Each row shows a region in which Scholars are currently living (e.g. Sub-Saharan Africa): the percentages shown are the proportion of Scholars currently living in that region who are also living within their home country or region.

An example: a Nigerian living in Nigeria would have a Residency of 'Sub Saharan Africa' and would be both living in their home country and region and so would be included in both columns of the table below. A Nigerian living in Kenya would _not_ be living in their home country but _would_ be living in their home region, so they would be included in the 'region' column but not the 'country' column. A Nigerian living in the UK would be living in neither their home country nor region and so would not be included in either column.

```{r, echo=FALSE}
bind_rows(rescountry_resreg,resregion_resreg) %>% 
  filter(Response=="Home") %>% 
  select(-freq, -Response, "Residency Region"=ResidencyRegion) %>% 
...
```{r, echo=FALSE}
empcurrent_resreg %>% 
  select(-freq,-Variable,ResidencyRegion) %>% 
  mutate(Response = fct_recode(Response, "Other" = "Neither employed nor studying")) %>% 
  spread(Response, prop) %>% 
  select("Residency Region"=ResidencyRegion, Employed, contains("Employed and studying"),Studying, Other) %>% 
  pander
...
```{r, echo=FALSE}
empsector_resreg %>% 
  select(-freq,-Variable, "Residency Region"=ResidencyRegion) %>% 
...
acapostaward_resreg %>% 
  filter(Response=="Yes") %>%
  arrange(desc(prop)) %>% 
  select( -Response,-Variable, "Proportion (%)"=prop,"Cases"=freq, "Residency Region"=ResidencyRegion) %>% 
  pander
```
...
```{r, echo=FALSE}
acaquallevel_resreg %>% 
  select(-freq, -Variable, ResidencyRegion, "Degree studied"=Response) %>% 
  spread("Degree studied",prop) %>% 
  select("Residency Region"=ResidencyRegion, Doctorate,contains("Master's degree"),contains("Professional accreditation"),contains("Technical training"),
...
```{r, echo=FALSE}
bind_rows(ldrbudget_resreg, ldrmanaging_resreg, ldrsupervise_resreg,ldrstrategy_resreg) %>% 
  filter(Response=="Yes") %>% 
  select(-freq, -Response,"Residency Region"=ResidencyRegion) %>% 
...
resmain_resreg %>% 
  filter(Response=="Yes") %>%
  arrange(desc(prop)) %>% 
  select(-Response,-Variable,"Residency Region"=ResidencyRegion,Cases=freq, "Proportion (%)"=prop) %>% 
  pander
```
...
```{r, echo=FALSE}
bind_rows(reseditor_resreg,resreviewer_resreg,resorganiser_resreg) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>%
  spread(Variable, prop) %>% 
  select("Residency Region"=ResidencyRegion,"Journal Editor"=ResEditor,"Peer reviewer"=ResReviewer,"Conference organiser" = ResOrganiser) %>%
  pander
...
```{r, echo=FALSE}
rbind(teachschool_resreg,teachundergrad_resreg,teachpostgrad_resreg,teachdoctorate_resreg,teachtvet_resreg) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>% 
  spread(Variable,prop) %>% 
  select("Residency Region"=ResidencyRegion,Doctoral=TeachDoctorate,Postgraduate=TeachPostgrad,
...
```{r, echo=FALSE}
bind_rows(impinstitutional_resreg,implocal_resreg,impnational_resreg,impinternational_resreg) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>% 
  spread(Variable, prop) %>% 
  select("Residency Region"=ResidencyRegion, Institutional=ImpInstitutional,Local=ImpLocal,
         National=ImpNational,International=ImpInternational) %>%
  pander
...
```{r, echo=FALSE}
bind_rows(impsocial_resreg,impcivic_resreg,impecon_resreg,imppolicy_resreg) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>% 
  spread(Variable, prop) %>%
  rename("Residency Region"=ResidencyRegion,"Civic Engagement"=ImpCivic, "Economic Development" = ImpEcon,
...
```{r, echo=FALSE}
bind_cols(select(indexskills_resreg,"Residency Region"=ResidencyRegion, Skills=Mean),
          select(indexldr_resreg, Leadership=Mean),
          select(indexresearch_resreg, "Research productivity"=Mean),
          select(indexcollab_resreg, "Research collaboration"=Mean)) %>% 
  pander
...
rmarkdown::render("2016_sch_long_residency.Rmd",output_format='pdf_document')
