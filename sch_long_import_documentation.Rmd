---
title: 'Documentation: Longitudinal Framework basic analysis'
output:
...
This output is based on an [R Markdown](http://rmarkdown.rstudio.com) Notebook. When you execute code within the notebook, the results appear beneath the code. If you are reading the HTML or Word versions, the code is printed but (obviously) does not work interactively. In the document, the code for accessing survey data and performing basic tabulation for a specific iteration of alumni surveys is set out. There are several associated R Markdown documents that take the data tables produced and turn them into, for instance, basic analyses for key variables (gender, nominating route etc.) and response rates. 

Three notes before you read on:  

1. 'tables' in this case means data frames within the R environment. Although you could theoretically export each table to MS Excel (or equivalent), that is not the purpose of this script: it is designed to automate 'mass production' of basica analysis with no / little manual manipulation of tables.  
2. The actual data for this analysis comes from a database (EvalDb) stored on the CSC virtual drive, so you will only be able to get access to the data (and run this code correctly) if you have access to the database on the CSC / ACU network
3. If you do not understand R code it is very likely that much of the document below will be confusing. Take heart: I understood none of this before I started, but am convinced it can save huge amounts of time and effort (especially on routine tasks) if a little time is invested in becoming familiar (use [Stack Overflow](https://stackoverflow.com/tags/r/info)!)

The remainder of the document goes through each stage of getting data from the evaluation database, cleaning it up to be a more R-friendly format, and conducting many basic analyses in which we might be interested. If you are not concerned with how data is accessed then you need only read the variables section (1) and then move on to the basic analysis section (x). It is worth reading everything at least once though, to understand how the analysis is conducted. If you arereading through with the intention to understand the code, note that this is written using the workflow and operators from [dplyr](http://dplyr.tidyverse.org/), not base R: watch out for pipes (%>%) etc.

# [1] Global Variables

Update these values in quotation marks to the correct SurveyIDs for your longitudinal follow-up and baseline survey. It is important that these values are set correctly: they are used in querying the database for the information, so if you give the wrong SurveyID (or a meaningless SurveyID) then you will get either the wrong data or none at all. 
...
The SurveyName variable is used only in some titles on alumni survey outputs, it is not vital for data access: put whatever you to appear as the description for the survey. There is not an equivalent for the baseline survey at the moment because there are no key variable analysis documents, but you could easily add 'SurveyName_base' and declare a variable for it (in the style above).

# [2] Library calls

The next step is to call the various packages that will help us access and analyse the data. The easiest way to do this is through pacman, a useful package-managing package (!). The first line of code checks if you have pacman installed and, if not, installs it from [CRAN](https://cran.r-project.org/). The second line calls the package itself and loads it. The third line uses pacman's p_load function to check and load the other packages used in this script.   
...
[RODBC](http://www.statmethods.net/input/dbinterface.html) is a database access package based on ODBC.
[Tidyverse](http://tidyverse.org/) loads a handful of packages from Hadley's excellent tidyverse. Many packages are installed, but we are going to use dplyr, forcats, and stringr in this script. We have to load the latter two separately because, although tidyverse installs them, they are not loaded automatically by tidyverse.

# [3] Importing and cleaning data

### [3.1] Data connection and import

Next, we need to actually get our data. This part takes quite a bit of coding because we are going to ask for something quite particular - we will ask R to get data from:

1. Libra
2. Evaluation database survey results
3. Evaluation database geographical regions data

Then to clean up that data, where needed, and then join it all together in one large 'table' (data frame) for our use. Moreover, we will ask for all of this information for the specific surveys we declared above, in the global variables section.

First, we need to make the database connections using the odcbcConnect function of the evaluation database package.
...
If that does not work then you need to check you have data source names on your computer for the two databases: speak to Arvinder.

Next, we query Libra for our 'population' data. This is the data that we are going to use for cross-referencing, to get administrative details for the survey respondents, but it is easier to do this first because it requires a little attention due to some of the ambiguities of the admin data.

We will take the query in five stages. First, send an SQL query to Libra for the data we want. Our query data is going to be limited to the year 2000 onwards because this removes some problems with legacy data: all survey respondents should be post-2000 Scholars anyway.
...
This should all be pretty familiar if you know SQL, otherwise check with someone who does know if you want to understand in more detail.

Stage two is a check of the admin records. We need to look at whether there are any duplicates that need correcting in Libra or are fine to exclude here. Pay close attention to the PhD duplicates.

```{r db_conn_libra_2}
population %>% filter(duplicated(population$AWDID)) %>% select(-starts_with("Jacs")) %>% tbl_df
```

If there is no problem with duplicates (there may be duplicates, but as long as they have the same information for each record it is fine) then we refine to have only unique records.

```{r db_conn_libra_3}
population <- population %>% filter(!duplicated(population$AWDID)) %>% tbl_df
```

At this point we should have a unique dataset with most of the Libra data we need. We have only two more variables we need to add: schemetype and committe group (ctteegroup). The first is used directly in our analysis, the second is an intermediary variable we need to work out our committee scores (see below).

So to move to stage three, we define the list of historical doctoral codes from the LKUPDEGREE table in Libra so that we can identify doctoral scholars, and then we use that with Sarah Humphrey's SchemeType formula to assign all of the records a SchemeType.
...
Note that this is just an R implementation of the formula Sarah wrote in Crystal SQL.

For stage four, we need to add the committee group and calculate a relative committee score that we can use in our analysis. Committee group is not a 'real' concept in CSC processes, it is only a way of expressing the idea that we are going to group together all of the Scholars who competed for the same funding. For example, all general PhD applications in a year, all developed country scholarships, etc. We group by competition and by year, because the quality of candidates varies by year and so we do not want to compare across different cohorts. Finally, we calculate a standard score ([z-score](https://en.wikipedia.org/wiki/Standard_score)) which expresses the difference between candidates in the same appliation committee group and same application year as a number of standard deviations above or below the mean (average) score.
...
Two points on the definition of standard scores: 1) scores of 0 are excluded as these generally represent ineligible candidates (too low on one criterion) and are not actively considered at committee; 2) scholarship applicants in schemes that do not use selection scores (e.g. Distance Learners) will simply not be assigned a score. At this point the standard scores should be fairly normally distributed, but later we will be using these scores with selected candidates (who, by definition, did well in the selection process) and so the scores will almost all be *positive*.

To complete our database query for Libra data, we need to remove our various intermediary variables.

```{r db_conn_libra_6}
population <- select(population,-DegreeCode, -PhD)
```

That is the Libra part of our database query completed - now we need some survey data!

First, we will get the alumni survey data. Again, this is a two step process because our our +2 year survey has very slightly different variables to our other surveys, meaning we need to import all of the data and then do a join operation to make the data frame we need.

Earlier - in [1] Global Variables - we defined the SurveyIDs we needed. The reason for that step should become clear here. We will query EvalDb in much the same way as Libra, but we actually need to write several small queries and then join the results together, because each set of survey data is in its own table. To make our query reusable, we are going to ask R to compile an SQL statement out of two components: the actual SQL expression we want and the SurveyID we defined earlier as a parameter. This is handled by the 'sprintf' function for each of the five queries: essentially it allows us to leave 'blanks' in the query and fill them in with variables we choose (either those defined in Global Variables, above, or some we are going to name on the fly below).

This time we will define our query first, to make it a bit easier to read, and we will again use our SurveyIDs from the [1] Global Variables section to make sure we get the right data. Then we will use the query we have defined to get the data from each table we need in EvalDb.
...
```
Note that we have asked R to make five queries (the sqlQuery part), create a table with the same columns and lots of rows from the first four (bind_rows part), and then join the that large table onto our +2 year survey results while keeping the columns from both (full_join part). After all of that, we have all of the survey results from the +2, +4, +6, +8 and +10 surveys for the SurveyIDs we are defined earlier, all joined together in one large data frame. 

Now we are in a position to join our survey data to our population data from Libra, so we can use a normal SQL left_join to keep all of the information from our survey data and the matching information from our population data, based on common AWDID. To support our analysis we also need some geographical data about the regions in which the countries of origin and residence are location, so we will add that data too by using another query that grabs the information from EvalDb and joins it to our new alumni dataset.
...
Note that in the last steps we have remamed the region variables and our current job sector variable to be more informative.

We can do exactly the same process for the baseline data, much simplified because there is only one table of baseline data in EvalDb.
...
We now have the full alumni and baseline datasets with all the Libra data and EvalDb data we need for our analysis. 

For the alumni data in particular, we might also want to do some analysis on response rates. To add response rates data we will need to another query to get the relevant data from our respondents table. Again, we will define our query first, to make it a bit easier to read, but this time we will also use a string (text) editing function to remove superfluous punctuation marks that has been introduced by our choice to split the query across multiple lines to keep it readable.
...
Now that we have defined the query, we can use it in the sqlQuery function to get our response data. Again, we can join our response rates data to the population data from Libra to give us all the extra information on our respondents that we might want to use in an analysis of response rates.
...
So now we have several data frames: 1] population data from Libra, 2] complete alumni survey data joned to relevant admin data, 3] our complete baseline survey data joined to relevant admin data, 4] response data from alumni survey joined to relevant admin data.

We will actually remove the population dataset later, since it was just an intermediary to get the information we needed for our survey recipients. For now, however, we will close the connection to our databases and move on to the next stage of importing and cleaning the data.
...
At this point we have data from the various databases in roughly the format we want, but we are missing some calculated variables not stored with the survey data and we also need to do some cleaning to make our data 'R-friendly'. We start by adding the additional variables we would like.

First, we add the residency status variables that are a simple comparison of whether someone is currently living in their home country and/or home region.
...
Next, we need to add the index variables which aggregate several groups of survey questions and allow easier comparison: see separate documentation on how these are calculated (or work it out from the code). To begin, we need to define some helper functions that will convert the labels ('rarely', 'never', etc.) into numbers on which we can perform calculations.
...
We apply these functions to the variables in the alumni data that we include in the indices and calculate the index scores - again, see separate documentation - and then merge the result back into the original alumni dataset using a left_join (keeping our four new columns in addition to all of the original survey data). Note that in line 13 we also replace the NA scores (because some people have no index score, due to the way they answer questions) with the lowest point on the scale, so no data is excluded in the calculation of indices.
...
The same for the baseline data.
...
These are all of the additional variables we should need for our basic analysis. Any other variables can, of course, be defined interactively while working with the data.

### [3.3] Setting variable types

To ensure our data behaves as we would expect, we need to set the variable types correctly. 

Many of our variables are ordered factors, so we will need to define some more helper functions to allow us to set those factors as we want, without going through each in turn. Note that each function is designed to factorise and assign an order to a different kind of factor variable used in our data.
...
Now we apply those factors to our data.
...
This sets our various ordered factors to be both factors and ordered. There are a couple of exceptions: we order three factors in the next code block, but they are unique and so not worth making a dedicated helper function for; ReturnIntention factor is an ordered factor as well, but it is easier to set this later because it also needs quite specific recoding of the factor labels (we return to it below). 

For the remainder of our factors, we need to set them as factors in R and to make any declarations about the levels to make sure no levels are omitted if there happens to be no data in a particular survey (e.g. no-one answered "Twice" to the CurrentJobChance question). Again, all of this is done fairly simply in one code block.
...
We do the same for the baseline data as well, although this time we will simply roll in the whole process to single block of code (note the ordered variables being set in the last three lines).
...
### [3.4] Recoding factor levels

Some factors have levels with unwieldy names that will make our analysis more difficult and any outputs (tables, graphs etc.) ugly. We can fix this in much the same way we dealt with setting variable types, by using the mutate function and some factor manipulation functions to change the levels from how they appear in the survey to how we would prefer they appear in the analysis
...
Note in the ln 3 of the code we use 'grepl', a string (text) search function, to effectively do a find and replace for anything within the ReturnIntention vaiable that starts with 'I never'. Then later, ln 14-20, set the rest of the levels for that factor and declare that it is ordered. This is why we mostly left ReturnIntention alone in [3.3] above.

Again, we do the same with our baseline data.
...
And for our response data. Note that for our response data we also make a slight alteration to the structure of the dataset, dropping the 'Scheme' variable because we have created the 'SchemeNom' (scheme nominating route) variables which is a more attractively labelled version of the same information.

```{r code_vars_3}
response.data <- 
  response.data %>% 
    mutate(SchemeNom = recode(Scheme, "CD"="Distance Learners","CR"="Agency: Developed","CA"="University Staff","CS"="Agency: Developing","CN"="Split Site","SS"="Shared Scholars") ) %>% 
    select(-Scheme, Response=ResponseName, OriginRegion=Region)
```

For the response data we can also do some minor renaming at this stage - originRegion = region etc. - to make our outputs more understandable.

### [3.5] Cleanup

At this stage we have imported all of the data we want to analyse, matched it with the relevant administrative data, added any additional variables we wanted to calculate in advance of our analysis, and cleaned up variable types and names to make them more amenable to clean and tidy analysis. We are finished importing, so we can remove all of our intermediate variables and leave us only with our three datasets (alumni, baseline, alumni responses) and our backup for the R graphical parameters (opar).
...
We can now began creating some basic descriptive statistics and proportion tables for our data. In the next section we focus only on the alumni data - including the response rates - but this can be easily generalised to the baseline data for those variables which are common to both alumni and baseline surveys.

First, we can view the structure of the dataset we have created

```{r alum_setup_1}
str(alumni.data, list.len=nrow(alumni.data))
```

So we can see that there are many survey response variables (i.e. answers to questions) and some subgroup or demographic variables (i.e. information about the respondent). To conduct our analysis we want to know what the responses to each question look like overall, and by each of several key variables (subgroups) in which we are routinely interest. Our subgroups will be:

1. Gender = gender
2. Scheme = sch
3. Scheme type = schtype
4. YearGroup = year
5. Origin region = orireg
6. Residency region = resreg
7. JACS category = jacs. Note: throughout, JACs categories included are limited to those with 20+ cases. JACs = NA is usually excluded.

To avoid having to many lines of identical code each time we conduct our analysis - since we will want the same breakdown for each survey response - we will again define some helper functions. These functions are a bit more complicated, so we will examine each in turn.

First, we need a function for our overall response to a survey question, which we will call 'population summary' (pop_summary). Our function will take four actions for us: 1) group a dataset we name by a variable that we name, 2) summarise the frequency of each response, 3) calculate the percentage of the total for each response, 4) make the resulting dataset look prettier.
...
When we run this function on a variable it will give us data in [long / narrow format](https://en.wikipedia.org/wiki/Wide_and_narrow_data). The argument 'dataframe' for the function is always the overall dataset, like alumni.data or response.data. The argument 'variable' is the field we actually want to do the work on, like 'CurrentSector'. In the code, ln 6-9 gives us the pretty data: the 'work' is done by ln 1-5.

Next we need a helper function for our subgroups (subgroup_summary). The structure of this function will be almost exactly the same, but we will add an extra argument to our helper function to allow us to define the group (subgroup) by which we also to divide the data up.
...
We still supply the 'dataframe', but now we give the grouping variable (e.g. Gender) before we specify what field we want to analyse. Again, the result will be in long format.

Finally, we will need a helper function to conduct some descriptive statistics on the standard score from the selection process that we calculated in [3.1] above. Routinely we will want the number of responses (frequency), some measures of central tendency (mean, median), dispersion (Standard Deviaton / SD), and range (max, min). This time, we do our filtering and grouping before we apply our function, so for the function itself we need only supply the dataframe.
...
The reason for only supplying the dataframe is that we are going to group by the variable we are interested in (e.g. CurrentSector), rather than a subgroup as we did before, which is harder to do as part of the function. See below for examples of how this has been done: in practice it only adds a single extra function call to each analysis.

Note that at this stage all of our analysis will be *univariate*. More complex analysis involving several variables should follow from conducting all of this (easy) univariate analysis, especially if, as would be usual, you are interested in potential interactions between subgroup or demographic factors.


### [4.1] Survey response rates

Before getting into the substantive analysis, we can generate some basic tables of survey response rates. Firstly, we can look at the overall response rates by using our pop_summary function and offering the 'Response', from the response.data, as our argument. Note that we have used the tilde (~) to show the 'Response' variable is from the  dataframe named in the first argument.

```{r alum_resp_1}
resp_overall <- pop_summary(response.data,~Response) %>% select(-Variable,Rate=prop)
```

We also drop the 'variable' column and rename the 'prop' column, to make the result prettier.

Because we also want to view our response rates by subgroup, we will use the subgroup_summary function we defined earlier. For each of our key variables we use basically the same code and assign the result to an appropriately named data frame.
...
Note that for the JACS categories we have filtered out any responses which are NA (not applicable: no category in this case) and those categories with 20 or fewer cases. This latter threshold is a matter of judgement: it could be set higher or lower, depending on how interested you are in the less voluminous categories and how will willing you are to deal with potentially misleading impressions from small numbers of cases.

Finally, we will use our score_summary function to examine response rates in relation to performance in the selection process.

```{r alum_resp_3}
resp_score <- response.data %>% group_by(Response) %>% score_summary()
```

As will be evident when you run this code, the result looks quite different to our pop_summary and subgroup_summary functions, but the idea is basically the same: generate some basic summary information to highlight any possible trends for further analysis.

### [4.2] Alumni survey data overview

Now we are at the point that we want to actually get some results from our alumni survey. First, we can examine what the distribution of survey respondents looks like in each of subgroups / key variables, so that we understand who actually replied to the survey.
...
Notice that these are almost exactly the same lines of code we used for our response data: we are just changing the input arguments and assigning the results to appropriately named variables. One potentially confusing factor here is the use of subgroups in the pop_summary function: usually we use subgroups in the *subgroup_summary* function! In this case, however, we are actually interested in the Gender, SchemeType etc. as the topic of our analysis, not as an additional way to divide the date from another question or topic. We want to know what the proportion of males and females were in our survey data, not how male and female respondents different in their responses to some other question. To achieve the desired result, we use the pop_Summary function.

### [4.3] Residency

We will prefix all of our output data frames related to residency with the prefix 'res', so we can easily find them.

First we look at residency variables overall.
...
Then we examine residency status for living in the *home country* for our subgroups.
...
And finally, the same examination of residency status for living in the *home region*.
...
This is the same principle we will use for all of the data: examine overall, then examine each individual variable (usually a single survey question) by each of our subgroups. By running the code, R will generate all of these data frames for our use: like creating many dozens of tables in MS Excel.

### [4.4] Employment

We will prefix all of our output data frames related to employment with the prefix 'emp', so we can easily find them. The employment context variables (e.g. variables that begin 'cxt') are not included here as these are meant for more complex analysis that compares the response on, for instance, application of skills questions to the supportiveness of the workplace environment reported by Scholars.

As before, we begin with overall figures.
...
empreturntime_overall <- alumni.data %>% filter(grepl("_Two$", SurveyID), !ReturnOrganisation=="Yes", CurrentEmploy=="Employed") %>%
                          pop_summary(~ReturnEmploy)
```

For the final two variables - return to organisation and return to work time - we need to do some additional filtering to include only those filling in the +2 survey and, in the case of the latter variable, those who are currently employed but *did not* return to their pre-scholarship organisation. These 'flows' of questions are detailed elsewhere, notably in the surveys themselves, and so will not be explained in detail in this document.
...
Skill match with current employment. Here we need to add an extra filter because we do not want to include cases where there is no answer to the CurrentSkillMatch question (i.e. the respondent is not employed).
...
empskillmatch_jacs <- alumni.data %>% filter(!JacsCat=="NA",!CurrentSkillMatch=="NA") %>% subgroup_summary(~JacsCat,~CurrentSkillMatch) %>%
                        filter(sum(freq)>20)
empskillmatch_score <- alumni.data %>% filter(!CurrentSkillMatch=="NA") %>% group_by(CurrentSkillMatch) %>% score_summary()
```

Changes of employment in the last two years. Again, we want to exclude respondents who have not got an answer to this question: those who have not been employed in the last years.
...
empchange_jacs <- alumni.data %>% filter(!JacsCat=="NA",!CurrentJobChange=="NA") %>% subgroup_summary(~JacsCat,~CurrentJobChange) %>%
                  filter(sum(freq)>20)
empchange_score <- alumni.data %>% filter(!CurrentJobChange=="NA") %>% group_by(CurrentJobChange) %>% score_summary()
```

Finally, we examine the two variables that are only relevant to respondents to the +2 year survey. Return to previous organisation is explored first. Note that the subgroup analysis for YearGroup is omitted because it is meaningless for this variable (all respondents are from one YearGroup)
...
Fro the time to gain employment variable we need to define an additional helper function to make our filtering process less long-winded. We use subset rather than dplyr filter here (it proved to be a little easier) and we filter the data to keep only those who are from the +2 year survey, did not return to their pre-scholarship organisation, but are currently employed.
...
Then we apply our filter function alongside our subgroup_summary to examine the data.
...
From this point the process of creating data frames for the variables is basically the same, although sometimes the code is presented in a different order, so further documentation will focus only on what is *different* to the process we have for residency and employment. 

### [4.4] Further qualifications

Prefix = "Aca". Collated here is code for creating data tables for the acapostaward, acaquallevel, and acacontribution questions, all concerning qualifications after completing a Commonwealth Scholarship. 
...
By Subect Studied / JACS category
...
Prefix = "ldr" and "Inn". Questions relating to leadership (ldr) and innovation (ldrinn)
...
### [4.6] Skill application

Prefix = "App" and "Skill". Two distinct gorups of questions here: 1) the skill gain questions, asked only of those filling in +2 year survey, 2) the application of skills questions, asked of everyone.

First, we look at all of the skill gains questions overall
...
Second, at all of the application of skills questions overal
...
Then at both sets of variables by each subgroup. YearGroup is omitted for the skill gains questions because it is not meaningful: the question is only asked of respondents who are answering the +2 survey.
...
YearGroup, for application of skills only
...
Prefix = "res". Questions are grouped into the overall research filter question, the collaboration questions, the academic community questions, and the research productivity questions.

Note that there are usually many NA responses for research variables because these questions are only asked to those that report current involvement in research: the filter question for research involvement is 'ResMain'. No reply (NA) for ResMain is usually because unemployed and studying respondents are not presented with the ResMain question, but it is worth checking to make sure there is no error in the data.
...
### [4.9] Networks and links

Prefix= 'Net'. Two sets of questions: networks (net) and network influence (netinf)
...
### [4.11] Analytic indices

prefix = "i." 

Overall statistics for indices can be created by assigning the index variables to their data frame and then applying a basic calculation of summary (the base R function) and SD across the new data frame of indices.
...
No helper function has been created yet for summarising index scores, so the code in this section is somewhat longer. Note that the summarise function is the same every time, but it works on a different index.
...
Committee Score is much harder to show in a table because both sets of variables are continuous, so it is better to use a scatterplot. These scatterplots are created using the [ggplot2](http://ggplot2.tidyverse.org/reference/) package and have a small jitter effect added so the points do not overlay quite as much.
