...
```{r echo=FALSE}
filter(resp.scheme, Response == "Completed") %>% 
  arrange(desc(Rate)) %>% 
  pander()
```
...
As noted above, there were `r select(resp.overall,n)[resp.overall$Response =="Email.failed",]` failed email addresses (`r (select(resp.overall,Rate)[resp.overall$Response =="Email.failed",])`%). The tabs below give a breakdown of failed emails by various categories.
...
```{r echo=FALSE}
filter(resp.YearGroup, Response == "Email.failed") %>% 
  arrange(desc(Rate)) %>% 
  pander()
```
...
```{r echo=FALSE}
filter(resp.scheme, Response == "Email.failed") %>% 
  arrange(desc(Rate)) %>% 
  pander()
```
...
```{r echo=FALSE}
filter(resp.phd, Response == "Email.failed") %>% 
  arrange(desc(Rate)) %>% 
  pander()
```
...
```{r echo=FALSE}
filter(resp.country, sum(n)>30, Response=="Email.failed") %>% 
  arrange(desc(Rate)) %>% 
  pander()
```
...
filter(resp.region, Response == "Email.failed") %>% 
  arrange(desc(Rate)) %>% 
  pander()
```
