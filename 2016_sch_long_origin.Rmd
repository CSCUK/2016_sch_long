---
title: '2016 Alumni surveys: Origin Region Breakdown'
author: "CSC E&M Team"
...
```{r, echo=FALSE}
overview_orireg %>% 
  select(-Variable,Origin=Response,Cases=freq, 'Proportion (%)'=prop) %>% 
  pander
```
...
```{r, echo=FALSE}
bind_rows(rescountry_orireg,resregion_orireg) %>% 
...
```{r, echo=FALSE}
bind_rows(ldrbudget_orireg, ldrmanaging_orireg, ldrsupervise_orireg,ldrstrategy_orireg) %>% 
  filter(Response=="Yes") %>% 
...
```{r, echo=FALSE}
bind_rows(reseditor_orireg,resreviewer_orireg,resorganiser_orireg) %>% 
  filter(Response=="Yes") %>% 
...
```{r, echo=FALSE}
rbind(teachschool_orireg,teachundergrad_orireg,teachpostgrad_orireg,teachdoctorate_orireg,teachtvet_orireg) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>% 
  spread(Variable,prop) %>% 
...
```{r, echo=FALSE}
bind_rows(impinstitutional_orireg,implocal_orireg,impnational_orireg,impinternational_orireg) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>% 
  spread(Variable, prop) %>% 
...
```{r, echo=FALSE}
bind_rows(impsocial_orireg,impcivic_orireg,impecon_orireg,imppolicy_orireg) %>% 
  filter(Response=="Yes") %>% 
  select(-freq,-Response) %>% 
  spread(Variable, prop) %>%
...
```{r, echo=FALSE}
bind_cols(select(indexskills_orireg, "Origin"=OriginRegion, Skills=Mean),
          select(indexldr_orireg, Leadership=Mean),
          select(indexresearch_orireg, "Research productivity"=Mean),
          select(indexcollab_orireg, "Research collaboration"=Mean)) %>% 
  pander
