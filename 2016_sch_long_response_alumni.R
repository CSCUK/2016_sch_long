...
load("2016_response_alumni.rdata")
...
plot_ly(filter(resp.region), x=filter(resp.region, Response=="Completed"), y=resp.region$Rate, type="bar")

x <- ggplot(data = filter(resp.region, Response=="Completed"), aes(x=reorder(Region,Rate), y=Rate))+
