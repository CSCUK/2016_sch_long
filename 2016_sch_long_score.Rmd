...
```{r, echo=FALSE}
bind_rows(rescountry_score,resregion_score) %>% 
...
```{r, echo=FALSE}
bind_rows(ldrbudget_score, ldrmanaging_score, ldrsupervise_score,ldrstrategy_score) %>% 
  filter(Response=="Yes") %>% 
  select(-freq, -Response,Subject=JacsCat) %>% 
