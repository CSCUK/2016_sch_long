...
This interpretation of this table is more complex for this variable, explanatory notes follow: 
Each row shows a region in which Scholars are currently living (e.g. Sub-Saharan Africa): the percentages shown are the proportion of Scholars currently living in that region who are also living within their home country or region. 
...
bind_rows(rescountry_resreg,resregion_resreg) %>% 
  ...
  select(-freq, -Response, "Residency Region"=Response) %>% 
...
```{r, echo=FALSE}
empcurrent_resreg %>% 
  select(-freq,-Variable, Origin = OriginRegion) %>% 
  mutate(Response = fct_recode(Response, "Other" = "Neither employed nor studying")) %>% 
  spread(Response, prop) %>% 
  select(Origin, Employed, contains("Employed and studying"),Studying, Other) %>% 
  pander
```
...
acapostaward_resreg %>% 
  filter(Response=="Yes") %>%
  arrange(desc(prop)) %>% 
  select( -Response,-Variable, "Proportion (%)"=prop,"Cases"=freq, Origin = OriginRegion) %>% 
  pander
```
...
```{r, echo=FALSE}
acaquallevel_resreg %>% 
  select(-freq, -Variable,  Origin = OriginRegion, "Degree studied"=Response) %>% 
  spread("Degree studied",prop) %>% 
  select(Origin, Doctorate,contains("Master's degree"),contains("Professional accreditation"),contains("Technical training"),
...
bind_rows(ldrbudget_resreg, ldrmanaging_resreg, ldrsupervise_resreg,ldrstrategy_resreg) %>% 
  filter(Response=="Yes") %>% 
  select(-freq, -Response,  Origin = OriginRegion) %>% 
...
resmain_resreg %>% 
  filter(Response=="Yes") %>%
  arrange(desc(prop)) %>% 
  select(-Response,-Variable,  Origin = OriginRegion,Cases=freq, "Proportion (%)"=prop) %>% 
  pander
```
...
bind_rows(reseditor_resreg,resreviewer_resreg,resorganiser_resreg) %>% 
  ...
  spread(Variable, prop) %>% 
  select(Origin = OriginRegion,"Journal Editor"=ResEditor,"Peer reviewer"=ResReviewer,"Conference organiser" = ResOrganiser) %>%
  pander
...
rbind(teachschool_resreg,teachundergrad_resreg,teachpostgrad_resreg,teachdoctorate_resreg,teachtvet_resreg) %>% 
  ...
  select(-freq,-Response) %>% 
  spread(Variable,prop) %>% 
  select(Origin = OriginRegion,Doctoral=TeachDoctorate,Postgraduate=TeachPostgrad,
...
bind_rows(impinstitutional_resreg,implocal_resreg,impnational_resreg,impinternational_resreg) %>% 
  ...
  select(-freq,-Response) %>% 
  spread(Variable, prop) %>% 
  select( Origin = OriginRegion, Institutional=ImpInstitutional,Local=ImpLocal,National=ImpNational,International=ImpInternational) %>%
  pander
...
bind_rows(impsocial_resreg,impcivic_resreg,impecon_resreg,imppolicy_resreg) %>% 
  ...
  select(-freq,-Response) %>% 
  spread(Variable, prop) %>%
  rename(Origin = OriginRegion,"Civic Engagement"=ImpCivic, "Economic Development" = ImpEcon,
...
```{r, echo=FALSE}
bind_cols(select(indexskills_resreg, Origin = OriginRegion, Skills=Mean),
